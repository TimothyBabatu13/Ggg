...
const getVarainceOfNumbers = (firstNum, secondNum) => {
  ...
  let sum = 0;
  for(let i = fff; i <= sec; i++){
    const subtractedValue = i - getMeanOfNumbers(fff, sec);
    const sqauredValue = subtractedValue * subtractedValue

    sum += sqauredValue;
  }

  const no = getNumberOfData(fff, sec) - 1;
  const result = sum/no
  return result
}
...
async function handle_advance(data) {
  ...
  const result = getVarainceOfNumbers(firstNumber, secondNumber);
...
async function handle_inspect(data) {
  ...
  const responseObject = "Inspect route not implemented for variance of numbers";
